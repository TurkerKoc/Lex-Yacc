...
{
       0,     0,     3,     5,     8,    13,    18,    21,    24,    26
};

/* YYRHS -- A `-1'-separated list of the rules' RHS.  */
static const yytype_int8 yyrhs[] =
{
       9,     0,    -1,    10,    -1,    10,     9,    -1,     4,     5,
      11,     7,    -1,     4,     5,     3,     7,    -1,     4,    11,
      -1,     6,    11,    -1,     4,    -1,     6,    -1
};
...
{
       0,    58,    58,    60,    63,   116,   129,   133,   137,   141
};
#endif
...
{
  "$end", "error", "$undefined", "EPSILON", "NONTERMINAL", "ASSIGN",
  "TERMINAL", "NEWLINE", "$accept", "program", "statement", "righths", 0
};
#endif
...
#line 64 "project.y"
    {
			printf("%s %s ",(yyvsp[(1) - (4)].str),(yyvsp[(2) - (4)].str));
			for(int i=0;i<RHS.size();i++)
			{
				cout<<RHS[i]<<" ";
			}
			cout<<endl;
			if(RHS[0]!=(yyvsp[(1) - (4)].str)) //if there is no left recursion
			{
				cout<<"checking isLRExist\n";
				int res=isLRExist(string((yyvsp[(1) - (4)].str)));//if lr of current nonterminal is already exist it will return index in leftRecursions
				...
				else //Left recursion of current nonterminal is not exist so directly write into output file
				{
					...
					writeDirectly(string((yyvsp[(1) - (4)].str))); //write current nt directly to the output file
				}
			}
			else //if current nonterminal is leftRecursion
			{
				cout<<"adding LR\n";
				int res=isLRExist(string((yyvsp[(1) - (4)].str)));//if lr is already exist it will return index in leftRecursions
				...
				else //new left Recursion
				{
					...
					addLR(string((yyvsp[(1) - (4)].str))); //add new lr into leftRecursions vector
				}
...
#line 117 "project.y"
    {
			...
			writeLinetoFile(string((yyvsp[(1) - (4)].str))+" -> epsilon"); //write line directly to the output file
...
#line 129 "project.y"
    {
			RHS.insert(RHS.begin(),string((yyvsp[(1) - (2)].str))); //push current lines rhs nonterminals and terminal into RHS vector
		}
    break;
...
#line 137 "project.y"
    {
			RHS.insert(RHS.begin(),string((yyvsp[(1) - (1)].str)));//push current lines rhs nonterminals and terminal into RHS vector
		}
    break;
